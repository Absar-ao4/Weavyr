...
collaborationRouter.get("/", authMiddleware, async (c) => {
    ...
    // Switch from 'include' to a top-level 'select'
    select: {
        id: true,
        createdAt: true,
        user1Id: true,
    ...
    orderBy: {
       ...
       const formattedCollaborations = collaborations.map((collab : any) => {
    // FIX: Grab the included user object
    const otherUser = collab.user1Id === userId ? collab.user2 : collab.user1;
    return {
        id: collab.id,
        user: otherUser, 
